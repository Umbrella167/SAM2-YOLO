...
class SAM2:
    ...
    # def load_batch(self):
    #     predict_dir = self.predict_path.split("/")[-1]
    #     if not os.path.exists(self.predict_path):
    #         os.makedirs(self.predict_path)
    #     image_list = [file for file in os.listdir(self.video_dir) if file != predict_dir]
    #     image_list.sort()
    #     len_image_list = len(image_list)
    #     batch_size = len_image_list if self.batch_size == -1 else self.batch_size
    #     for i in range(0, len_image_list, batch_size):
    ...
    #         for id in range(len(out_obj_ids)):
    #             print()
    #             binary_mask = self._logits2mask(out_mask_logits,[out_obj_ids[id]])
    #             video_segments[out_frame_idx][out_obj_ids[id]] = binary_mask
    ...
    def generate_yaml_file(
        self, num_classes, class_names, yaml_path="yolo.yaml"
    ):
        """
